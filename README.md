# antcatena-commits.github.io
Portfolio Data/BI Analytics 
# 🔥 Analyse des Feux de Forêt aux États-Unis

Projet Data complet utilisant **Python (Google Colab)** et **Power BI** pour analyser les feux de forêt aux USA entre 1992 et 2015.

---


## 🎯 Objectifs du projet
- Identifier les zones les plus touchées  
- Analyser les causes principales  
- Étudier l’évolution temporelle  
- Construire un dashboard interactif
- Proposer des préconisations pour faire de la prévention dans les zones prioritaires

---

## 📊 1. Analyse & Visualisations (Google Colab)

### Prétraitement des données
![Prétraitement](images/colab_preprocessing.png)

### Visualisation des tendances
![Graphiques](images/colab_visuals.png)

---

## 📈 2. Dashboard Power BI

### Vue générale
![Dashboard Power BI](images/powerbi_dashboard.png)

### KPIs & insights
![KPIs](images/powerbi_kpis.png)



---

## 📂 Ressources
- Notebook Colab : [Lien](https://colab.research.google.com/…)  
- Dashboard Power BI : [Lien](https://app.powerbi.com/…)  

---

## 👨‍💻 Auteur
Antonio — Data Analyst
